...
object DietPromptBuilder {
    ...
    private fun buildPromptText(userNote: String, mealType: String, hasImage: Boolean): String {
        return buildString {
            ...
            appendLine("""{"foods":[{"name":"食物名称","estimatedCalories":150,"estimatedGrams":200,"category":"主食","quality":"OFTEN","protein":5,"carbs":30,"fat":3}],"totalCalories":350,"macros":{"protein":10,"carbs":15,"fat":12},"trafficLight":"GREEN","advice":"简短饮食建议（一句话，温暖鼓励的语气）","adjustedDescription":"调整说明（如有用户补充，说明如何调整的）"}""")
            appendLine()
            appendLine("【注意事项】")
            if (hasImage) {
            ...
            appendLine("- quality判断依据（三选一）：低油低糖高纤维为OFTEN；高油高糖但偶可接受为SOMETIMES；油炸食品、高糖饮品、暴饮暴食类为INDULGENT")
