 ...
 *               输入是轻量投影查询（getFoodJsonSince）的 JSON 字符串列表
 *@author: 杨帅林
 *@create: 2026/8/21
 **/
object FrequentFoodAggregator {
     ...
     * - 热量中位数优先 >0 样本，全为 0 时回退全样本（避免 chip 显示 0 kcal）
    ...
    fun topFoods(
        ...
        return groups.entries
            ...
            .map { (name, samples) ->
                RecognizedFoodItem(
                    name = name,
                    estimatedCalories = medianOfPositive(samples.map { it.calories })
                        ?: medianOf(samples.map { it.calories })
                        ?: 0,
