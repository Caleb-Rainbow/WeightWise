...
object BackupDeduplicator {
    ...
    fun filterNewRecords(existing: List<Record>, incoming: List<RecordBackup>): DedupResult<Record> {
        val existingKeys = existing.map { "${it.timestamp}|${it.weight}" }.toHashSet()
        ...
        for (item in incoming) {
            ...
            if ("${item.timestamp}|${item.weight}" in existingKeys) {
                skipped++
            } else {
    ...
    fun filterNewDietRecords(
        existing: List<DietRecord>,
        incoming: List<DietRecordBackup>,
        buildImageUri: (fileName: String) -> String,
    ): DedupResult<DietRecord> {
        val existingKeys = existing.map { "${it.date}|${it.timestamp}|${it.mealType}" }.toHashSet()
        ...
        for (item in incoming) {
            ...
            if ("${item.date}|${item.timestamp}|${item.mealType}" in existingKeys) {
                skipped++
            } else {
                toInsert.add(
                    DietRecord(
