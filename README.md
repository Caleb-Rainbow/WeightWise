...
- **周期报告** — 周报 / 月报 / 年报（自然周期）独立报告页：体重净变化、较上一期对比、打卡率、最高/最低/平均、趋势图与目标参考线；融合日均热量、超标天数与饮食红绿灯分布；支持左右翻页回顾历史周期；AI 深度总结按当前周期数据生成（超量记录自动按月聚合）
- **周报推送** — WorkManager 每周一固定时刻推送上周体重报告摘要（打卡天数、净变化、与前一周对比），点通知直达报告页
...
- **AI 体重分析** — 基于豆包大模型（火山方舟）流式输出周期分析，融合时间范围内的每日热量摄入与个人档案，能解读「为什么这周体重涨了」（入口位于报告页）
...
├── ui/                          # 表现层
│   ├── main/                    # 首页（体重记录、图表、目标进度、BMI）
│   ├── report/                  # 周期报告页（周/月/年报、翻页、AI 总结）
...
- **体重分析**：报告页按所选周期（周/月/年）的体重与日志数据构建 Prompt（记录超 62 条自动按月聚合防止超长），经豆包 API 流式生成三段式分析（阶段总结 / 数据洞察 / 行动建议）；请求失败在界面呈现错误态并支持重试。
